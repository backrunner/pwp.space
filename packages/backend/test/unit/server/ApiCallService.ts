...
function createService(options: {
	user?: { id: string; isSuspended: boolean; approved: boolean } | null;
	approvalRequiredForSignup?: boolean;
} = {}) {
	const authenticateService = {
		authenticate: vi.fn().mockResolvedValue([options.user ?? null, null]),
	};
	...
	const metaService = {
		fetch: vi.fn().mockResolvedValue({ approvalRequiredForSignup: options.approvalRequiredForSignup ?? false }),
	};
...
describe('ApiCallService structured error logging', () => {
	test('rejects an unapproved account when signup approval is required', async () => {
		const { service } = createService({
			user: { id: 'user-id', isSuspended: false, approved: false },
		...
		try {
			...
			expect(reply.send).toHaveBeenCalledWith({
				error: expect.objectContaining({ code: 'YOUR_ACCOUNT_NOT_APPROVED' }),
			});
