...
import { createNoteViewState, fetchLatestNote, replaceNoteState, resolveCollapsedStateAfterRefresh, resolveNoteStateAfterRefresh } from '@/utility/note-refresh.js';
...
export function useNote(
	...
	// 基本状態
	const initialNoteState = createNoteViewState(initialNote);
	const rawNote = reactive(initialNoteState.rawNote);
	const isRenote = computed(() => Misskey.note.isPureRenote(rawNote));
	const appearNote = reactive(initialNoteState.appearNote);
	...
	const { $note: $appearNote, subscribe: subscribeManuallyToNoteCapture } = useNoteCapture({
		...
		parentNote: rawNote,
		mock: props.mock,
		forceSubscribe: inTimeline || options.forceNoteCapture,
	});
	...
	const renoteCollapsed = ref(prefer.s.collapseRenotes && isRenote.value && (($i && ($i.id === rawNote.userId || $i.id === appearNote.userId)) || ($appearNote.myReaction != null)));
	// Keep refresh generations independent: a wrapper and its renote target can
	...
	useGlobalEvent('noteUpdated', async (noteId) => {
		if (noteId !== rawNote.id && noteId !== appearNote.id && noteId !== rawNote.renote?.id) return;
		const refreshRequest = (latestRefreshRequests.get(noteId) ?? 0) + 1;
		latestRefreshRequests.set(noteId, refreshRequest);

		try {
			const refreshedNote = await fetchLatestNote(noteId);
			if (refreshRequest !== latestRefreshRequests.get(noteId)) return;
			const wasLong = isLong.value;
			const wasCollapsed = collapsed.value;
			const wasRenote = isRenote.value;
			const updatedNoteState = resolveNoteStateAfterRefresh(rawNote, noteId, refreshedNote);
			if (updatedNoteState == null) return;

			replaceNoteState(rawNote, updatedNoteState.rawNote);
			replaceNoteState(appearNote, updatedNoteState.appearNote);
			if (!isRenote.value) {
				renoteCollapsed.value = false;
			} else if (!wasRenote) {
				renoteCollapsed.value = prefer.s.collapseRenotes && (($i && ($i.id === rawNote.userId || $i.id === appearNote.userId)) || ($appearNote.myReaction != null));
			}
			collapsed.value = resolveCollapsedStateAfterRefresh({
				wasLong,
				wasCollapsed,
				isLong: isLong.value,
